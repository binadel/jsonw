...
import (
	...
	json "github.com/binadel/jsonw/jsondi"
)

func TestJsondi(t *testing.T) {
	users := generateUsers(100)
	usersJson, _ := js.Marshal(users)
	usersJsondi := writeUsersJsondi(users)

	if string(usersJsondi) != string(usersJson) {
		t.Error("Users json are different")
	}

	posts := generatePosts(users, 50)
	postsJson, _ := js.Marshal(posts)
	postsJsondi := writePostsJsondi(posts)

	if string(postsJsondi) != string(postsJson) {
		t.Error("Posts json are different")
	}
}

func writeUsersJsondi(users []User) []byte {
...
func writePostsJsondi(posts []Post) []byte {
