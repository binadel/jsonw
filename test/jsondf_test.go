...
import (
	...
	json "github.com/binadel/jsonw/jsondf"
)

func TestJsondf(t *testing.T) {
	users := generateUsers(100)
	usersJson, _ := js.Marshal(users)
	usersJsondf := writeUsersJsondf(users)

	if string(usersJsondf) != string(usersJson) {
		t.Error("Users json are different")
	}

	posts := generatePosts(users, 50)
	postsJson, _ := js.Marshal(posts)
	postsJsondf := writePostsJsondf(posts)

	if string(postsJsondf) != string(postsJson) {
